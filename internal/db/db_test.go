...
func TestOpenCreatesSchema(t *testing.T) {
	...
	for _, column := range []string{"submitted_head_sha", "review_approved_head_sha", "test_verified_head_sha", "last_pushed_sha", "push_target_fingerprint", "push_ref", "last_pushed_at", "push_generation", "push_active", "pr_state", "pr_state_observed_at", "ci_ready_at", "custody_returned_at"} {
...
func TestOpenMigratesRunSyncProvenanceWithoutBackfillingMutableHead(t *testing.T) {
	...
	if run.TestVerifiedHeadSHA != nil {
		t.Fatalf("legacy run inferred a test-verified head from mutable head: %#v", run)
	}
