...
const branchSyncAgentGuidance = "Before a post-pipeline local commit or fresh run, follow the structured `branch_sync.next_action`. Run `no-mistakes axi sync` only when its code is `sync`; that guarded sync may be a strict fast-forward or a content-equivalent diverged advance that anchors the pre-sync head before moving the branch with reset semantics. Run `no-mistakes axi sync --recover` only when its code is `recover_custody` (a terminal run left unpublished pipeline commits preserved in the local gate under an exact run-owned ref); `no-mistakes rerun` resumes that head only when the exact ref and gate branch agree. When its code is `resolve_ambiguous_custody`, preservation evidence names more than one head and nothing may be chosen for you: relay the listed candidate commits to the user and run `no-mistakes axi sync --recover --resolve-head <commit>` only with the exact commit they name; every losing head is archived, never discarded. When its code is `apply_missing_preserved_commits`, an explicit `--keep-local` custody recovery refused because the current head is missing preserved pipeline commits by content: the note names each missing commit and the reported command only inspects them, so apply that missing work deliberately (relaying it to the user when it is not yours to apply) before re-running `no-mistakes axi sync --recover --keep-local`. Process blocked or pipeline-owned states instead of improvising reset, stash, merge, rebase, force, or branch replacement."
