...
- For PRs and CI: `gh` (GitHub), `glab` (GitLab), authenticated `bkt` v0.30.0+ or the direct REST credentials (Bitbucket Cloud), or `az` with the `azure-devops` extension (Azure DevOps)
...
8. **PR** - create the pull request or publish a validation notice on the existing one
