...
That byte-verified run-owned ref is the authority; the mutable gate branch is provenance and is never required to agree, so repeating the same recovery is idempotent rather than degrading into a weaker refusal.
