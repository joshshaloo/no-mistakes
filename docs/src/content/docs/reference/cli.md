...
When the gate's preservation refs cannot be read at all, ambiguity cannot be ruled out, so inspection reports `safety: blocked_preservation_unreadable` and both recovery and resolution refuse rather than treat the branch as unambiguous.
