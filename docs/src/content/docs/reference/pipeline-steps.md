...
- Each ready stale-risk notice names a verified provider-owned CI attempt identity. Before readiness, no-mistakes reconciles the current complete notice for that exact attempt and appends a replacement when it is absent or incomplete; it fails closed when attempt identity or notice state cannot be established.
- GitHub Actions is the only currently verified attempt-identity contract: jobs are checked against the repository, exact head, workflow run ID, and run-attempt number; generic check links are not identities. GitLab, Azure DevOps policy evaluations, Bitbucket Cloud's currently supported commit-status path, and unknown providers fail closed because those paths do not establish a non-reusable attempt identity.
