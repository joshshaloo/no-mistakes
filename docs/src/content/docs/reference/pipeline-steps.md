...
After each review round beyond the first that reports a finding, no-mistakes can record an advisory judgment on the run: whether the fixes already applied on this run have failed to settle the underlying cause behind the current finding, so that fixing again would prop up a design that keeps producing defects rather than close an independent defect. The run retains the latest successful measurement.
...
Later code, test, configuration, or executable changes invalidate the completed review's risk assessment, including files added by a CI repair. AXI reports `risk: stale`, and an already-open live TUI replaces the old rating with `STALE`. The PR conversation receives an append-only, exact-head validation notice saying that the changes are not covered by the previous assessment. Green CI (`checks-passed`) or a completed run does **not** renew that assessment. A supervisor must obtain a fresh review before relying on a rating to authorize merging. Original review rounds remain historical evidence; review, documentation, and lint are not automatically repeated.
...
**Behavior:**
- If `commands.test` is set in repo config: resolves any repository-declared Node pin according to the linked configuration contract, then runs the command as a baseline via the platform shell (`sh -c` on POSIX, `cmd.exe /c` on Windows) and captures output. An unresolved pin fails the step before the command runs; an ordinary non-zero command exit produces `error` findings. Configure a **targeted** command here (see repo-config); do not treat this field as CI-parity complete-suite configuration.
...
- Changed content is re-verified: `commands.test` is re-run when configured, with the same repository-pinned Node resolution described in the [configuration contract](/no-mistakes/reference/repo-config/#commandstest); otherwise the agent runs the workspace typecheck when the repository configures one, plus the smallest suites covering the files that changed after Test. The re-verification never edits files.
...
- Bitbucket Cloud has neither both direct REST credentials nor an installed, authenticated `bkt` v0.30.0+ Cloud Keychain context; a partial direct credential pair is also rejected explicitly
...
- If one exists, leaves its title, description, and comments byte-for-byte untouched; if not, creates a new one with its initial generated title and body
- Publishes pipeline status as separate append-only, exact-head notices in the PR conversation; these notices never overwrite human-authored comments
- Uses the provider CLI for GitHub/GitLab, the `az` CLI for Azure DevOps, and either authenticated `bkt` or the backward-compatible direct REST client for Bitbucket Cloud; complete direct credentials take precedence
...
- A newly created PR body includes a permanent pointer to its conversation notices, a `## Intent` section when user intent is available, an agent-authored `## What Changed`, and initial `## Risk Assessment`, `## Testing`, and `## Pipeline` sections from recorded step results and rounds; auto-fix results in `## Pipeline` render as an issue -> fix -> verification narrative using captured fix summaries, re-check success text, and any still-open findings
- Newly generated PR bodies are capped at 63,488 bytes, leaving a 2 KB safety buffer below GitHub's 65,536-character body limit.
- When a new body would exceed that cap, the PR step first omits older `## Pipeline` update rounds at clean update boundaries, keeps the newest rounds when possible, and points reviewers to the run log for the full pipeline history.
- Intent, `## What Changed`, risk, and testing sections are kept ahead of pipeline history; if those sections or the newest pipeline update are still too large during creation, the PR step truncates at line or section boundaries and adds an explicit marker.
...
- Bitbucket Cloud requires either authenticated `bkt` v0.30.0+ with a Cloud Keychain context or both `NO_MISTAKES_BITBUCKET_EMAIL` and `NO_MISTAKES_BITBUCKET_API_TOKEN`; complete direct credentials take precedence.
- Azure DevOps requires the `az` CLI with the `azure-devops` extension, authenticated with a PAT.

**Behavior:**
- Polls provider CI status at increasing intervals: every 30s for the first 5 minutes, every 60s for 5-15 minutes, every 120s after that. A rerun is externally visible on the next successful provider poll (so the normal observation window is the current interval); API outages have no bounded visibility guarantee and fail closed rather than preserving readiness.
...
- GitHub Actions is the only currently verified attempt-identity contract: jobs are checked against the repository, exact head, workflow run ID, and run-attempt number; generic check links are not identities. GitLab, Azure DevOps policy evaluations, Bitbucket Cloud's currently supported commit-status path, and unknown providers cannot authorize readiness because those paths do not establish a non-reusable attempt identity. This restriction fails closed on the readiness claim, not on repair work: failing checks from those providers can still be diagnosed and repaired, and a complete exact-head stale-review notice must be published before any repair push. That pre-push notice reports review applicability without claiming an external CI attempt.
...
- GitHub Actions job and workflow-attempt responses are each bounded during ingestion to 2 MiB of stdout and 32 KiB of diagnostics. Overflow, malformed data, cancellation, or a repository/head/attempt mismatch refuses readiness rather than parsing a truncated response.
...
- When stale-risk readiness requires attempt identity, waits up to 60s for initially empty check results to register, with readiness false and no ready notice. The window starts with CI monitoring and resets after a CI repair is pushed; empty polls do not extend it, poll sleeps are capped at its remaining time, and expiry without authoritative checks fails closed.
...
- On CI failure: fetches failed job logs (GitHub via `gh run view --log-failed`, GitLab via `glab ci trace`, Bitbucket Cloud via bounded failed pipeline step logs identified by the exact pipeline UUID, using the selected `bkt` or REST path; Azure DevOps has no first-class build-log command, so the agent fixes from the failing-check list without logs), sends them to the agent with user intent when available, and, if the agent produces changes, commits them and uses the same force-push safety guard as the push step
