...
- Each stale-risk notice names the provider-owned CI attempt identity. Before readiness, no-mistakes reconciles the current complete notice for that exact attempt and appends a replacement when it is absent or incomplete; it fails closed when attempt identity or notice state cannot be established.
- GitHub stale-risk readiness accepts only Actions jobs verified against the repository, exact head, workflow run ID, and run-attempt number; generic check links are not identities. Bitbucket Cloud's currently supported commit-status path does not expose a non-reusable attempt identity, so a stale assessment fails closed there rather than treating a reusable status UUID, key, timestamp, or URL as current-attempt proof.
