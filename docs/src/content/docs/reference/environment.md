...
The ID of the run that owns this process. On platforms where cleanup can inspect process environments, it proves a background process belongs to a run before signalling it, including after the process has been reparented away, moved to another directory, or outlived a daemon restart. See [Daemon & Worktrees](/no-mistakes/concepts/daemon/#what-it-does) for the lifecycle guarantee it supports.
