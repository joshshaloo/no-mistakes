...
A process that also escaped its command's process group - a dev server or dashboard that started a session of its own - is additionally caught on Linux and on Windows, where the kill-on-close job object tears the tree down.
On macOS that extra coverage does not exist yet, so an escaped process can outlive the run.
Every subprocess a run launches inherits a run marker in its environment, and Linux cleanup reaps a process group only when a member both sits in the run worktree and carries that marker.
The marker survives the escapee being reparented away and survives a daemon restart, which is what lets startup cleanup of an orphaned worktree still reap processes left behind by an earlier daemon.
An unmarked process is never signalled, so a shell or editor you opened inside a retained worktree is left alone.
