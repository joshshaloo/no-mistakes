...
A process that also escaped its command's process group - a dev server or dashboard that started a session of its own - is additionally caught on Linux, where cleanup discovers process groups whose members both sit in the run worktree and descend from the daemon, and on Windows, where the kill-on-close job object tears the tree down.
On macOS that extra discovery pass does not exist yet, so an escaped process can outlive the run.
Cleanup never signals a process outside the run's own tree, so a shell or editor you opened inside a retained worktree is left alone.
