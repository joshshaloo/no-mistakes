...
The marker is the whole proof of ownership, so it still identifies an escapee that reparented to init, moved to a session of its own, or changed directory out of the worktree entirely, and it survives a daemon restart - which is what lets startup cleanup of an orphaned worktree reap processes left behind by an earlier daemon.
An unmarked process is never signalled, so a shell or editor you opened inside a retained worktree is left alone, and a run is only ever matched by its own globally unique run ID, never by a sibling's.
