...
4. When the run finishes (success or failure), terminates its registered and platform-discoverable process groups first - `SIGTERM`, a bounded grace period, then `SIGKILL` for survivors, or a forced process-tree kill on Windows - so those processes cannot keep holding the worktree or a port. This happens even when the head check below retains the worktree
