...
- Every subprocess a run owns must also be registered with that run's supervisor, or it silently drops out of run-end reaping: `shellenv.ConfigureShellCommandForContext(ctx, cmd)` for short-lived Git and SCM calls (it isolates a process group only when ctx carries a supervisor, so a CLI or TUI Ctrl-C still reaches them), or `ConfigureShellCommand` plus `SuperviseShellCommand(ctx, cmd)` for agents and configured commands. `StartShellCommand` is the single boundary that stamps `shellenv.RunIDEnvVar` into the environment, so never start a run-owned command another way; a launch boundary that builds its own env (the managed agent server) calls `ApplyRunMarkers`. Escaped processes are reaped only through the platform ownership proof: the exact inherited marker on Linux, daemon ancestry plus cwd inside the worktree on macOS, or kill-on-close job membership on Windows. The `internal/shellenv/run_supervisor.go` doc comments own the rules; `docs/src/content/docs/concepts/daemon.md` owns the user-facing guarantee. Regressions: `TestConfigureShellCommandForContext_*`, `TestStartShellCommand_StampsRunMarkerOnSupervisedCommand`, `TestRunCleanupKillsBackgroundChildBeforeRemovingWorktree`, `TestRunCleanupDoesNotKillDaemonOrSiblingRun`, `TestRunCleanupReapsEscapedProcessThatLeftTheWorktree`, `TestOrphanWorktreeCleanupReapsEscapedProcess`, `TestDiscoverRunProcessesDarwinRequiresDaemonDescendant`, `TestRunSupervisorTerminateDarwinKillsDiscoveredDescendant`.
