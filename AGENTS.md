...
- A TERMINAL run with unpublished pipeline commits is recoverable, never a dead end: inspection reports `blocked_pipeline_owned_recoverable` + `next_action recover_custody`, and `sync --recover` fetches the byte-verified run ref to `refs/no-mistakes/recover/<run>` before stamping `runs.custody_returned_at`. That ref must equal `runs.head_sha` byte-for-byte, and the mutable gate branch must be the preserved head, the run's own immutable submitted head, or the kept head of an interrupted `--keep-local`; a pinned run ref never licenses displacing any other live gate head. Accepting an already-pinned ref in the submitted-head shape is what keeps repeated recovery idempotent; missing objects, third heads, unreadable preservation refs, and races remain blocked.
