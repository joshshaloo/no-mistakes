...
- A TERMINAL run with unpublished pipeline commits is recoverable, never a dead end: inspection reports `blocked_pipeline_owned_recoverable` + `next_action recover_custody` (ambiguous crash heads stay blocked), and `sync --recover` fetches the byte-verified run ref to `refs/no-mistakes/recover/<run>` before stamping `runs.custody_returned_at`. The legacy fallback is limited to clean local == gate == immutable submitted head, recorded object present in the gate, and no active owner; third heads, missing objects, and races remain blocked.
