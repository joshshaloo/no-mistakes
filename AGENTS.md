...
- Every subprocess a run owns must also be registered with that run's supervisor, or it silently drops out of run-end reaping: `shellenv.ConfigureShellCommandForContext(ctx, cmd)` for short-lived Git and SCM calls (it isolates a process group only when ctx carries a supervisor, so a CLI or TUI Ctrl-C still reaches them), or `ConfigureShellCommand` plus `SuperviseShellCommand(ctx, cmd)` for agents and configured commands. `StartShellCommand` is the single boundary that stamps `shellenv.RunIDEnvVar` into the environment, so never start a run-owned command another way; a launch boundary that builds its own env (the managed agent server) calls `ApplyRunMarkers`. That inherited marker is the only proof of ownership cleanup may signal on - never cwd or ancestry. The `internal/shellenv/run_supervisor.go` doc comments own the rules; `docs/src/content/docs/concepts/daemon.md` owns the user-facing guarantee. Regressions: `TestConfigureShellCommandForContext_*`, `TestStartShellCommand_StampsRunMarkerOnSupervisedCommand`, `TestRunCleanupKillsBackgroundChildBeforeRemovingWorktree`, `TestRunCleanupDoesNotKillDaemonOrSiblingRun`, `TestRunCleanupReapsEscapedProcessThatLeftTheWorktree`, `TestOrphanWorktreeCleanupReapsEscapedProcess`.
...
- `document.instructions` (the repo's documentation placement policy) and `disable_project_settings` (the gate-agent project-instruction opt-out) are also trusted-only: a pushed branch must not weaken either boundary. When the opt-out is enabled, only adapters with verified effective suppression may launch. Non-executing fields (`ignore_patterns`, `auto_fix`, `commit`, `intent`, `test`) are still read from the pushed branch.
...
- Every run pins exact authority at `refs/no-mistakes/run-head/<run>`; `git.PublishRunHead` owns preservation-before-branch-CAS ordering, and step code updates DB/in-memory authority only afterward. Normal and startup cleanup must prove the recorded head is pinned before removing a worktree; a differing live head is retained and separately named at `refs/no-mistakes/crash-head/<run>`, never promoted or discarded. Rerun consumes the exact verified run ref or refuses with custody guidance.
- A TERMINAL run with unpublished pipeline commits is recoverable, never a dead end: inspection reports `blocked_pipeline_owned_recoverable` + `next_action recover_custody`, and `sync --recover` fetches the byte-verified run ref to `refs/no-mistakes/recover/<run>` before stamping `runs.custody_returned_at`. That ref must equal `runs.head_sha` byte-for-byte, and the mutable gate branch must be explainable: the preserved head, the run's own immutable submitted head, the kept head of an interrupted `--keep-local` proven by this recovery's own kept-head intent ref, or - under explicit `--keep-local` only - a diverged gate head whose own unique commits AND every preserved commit are already present in the current head by patch content, which is archived before it is adopted as run authority. A pinned run ref never licenses displacing an unexplained live gate head. Patch containment is unprovable across a merge commit in a compared range, so that refuses too, and incomplete containment refuses with `blocked_recover_missing_preserved_commits` + `next_action apply_missing_preserved_commits` naming the exact missing commits. Accepting an already-pinned ref in the submitted-head shape is what keeps repeated recovery idempotent; missing objects, unexplained third heads, unreadable preservation refs, and races remain blocked.
- Ambiguous preservation evidence is blocked but not terminal: `blocked_pipeline_owned_ambiguous` + `next_action resolve_ambiguous_custody` requires `sync --recover --resolve-head <commit>`, which archives every losing head under `refs/no-mistakes/archive/<run>/<sha>` before CAS-publishing the operator-named commit to the run ref and `runs.head_sha`. Nothing is ever chosen automatically. An unreadable preservation snapshot is branch-scoped, not run-scoped, because it is also an input to run selection: `branchsync.PreservationUnreadable` owns that verdict, every presenter and fresh-run preflight must consult it (a state-only switch silently drops it), and recovery, resolution, and fresh runs all refuse. Startup retires a run ref only when the run is terminal, unambiguous, unowned, and either custody was returned or its exact head merged; crash and archive evidence is always retained.
  Equal/ahead worktrees anchor locally without gate access; behind/diverged worktrees fast-forward only when clean. `--keep-local` never touches the worktree, preserves both heads, and CAS-moves the gate branch only when needed via gate-side fetch - never a push, which would fire the receive hook.
...
- A green Test round atomically records `runs.test_verified_tree_sha` (the exact worktree content, not `HEAD`, so test-agent files a later step commits are not a post-test change); parked, failed, and skipped rounds record nothing. `internal/pipeline/steps/final_head_verification.go` is the single owner of the rule that a post-Test pipeline commit invalidates that evidence: the push boundary re-verifies against the durable anchor before network mutation, so a resumed run still re-verifies and an unchanged tree costs nothing. The verification is appended to the Test step's findings as a `post_test_fix_verification` round and never replaces its evidence; an unresolvable tree counts as changed. User-facing owner: `docs/src/content/docs/reference/pipeline-steps.md` (Push / Final head re-verification). Regressions: `internal/pipeline/steps/final_head_verification_test.go`, `TestCompleteTestStepRecordsVerifiedTreeAtomically`.
- A configured `commands.*` tool that is absent from the worktree (exit 127) must fail its owning step through `runConfiguredStepShellCommand`, never pass and never park: absence of a checker is not a check. Regressions: `TestLintStep_ConfiguredLintCommandNotFoundFailsStep`, `TestPushStep_MissingConfiguredFormatterFailsStep`.
